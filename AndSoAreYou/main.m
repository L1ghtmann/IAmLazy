...
int main(int argc, char *argv[]) {
    ...
    if(strcmp(argv[1], "copy-files") == 0 && argc == 3){
        ...
        */
        ...
        // install debs AND resolve dependencies (namely bootstrap packages)
        NSString *cmd = [NSString stringWithFormat:@"apt-get install -y --allow-downgrades --allow-unauthenticated .%@*", tmpDir]; 
        executeCommand(cmd);

        // make sure all packages are setup properly
        NSString *cmd2 = [NSString stringWithFormat:@"apt-get install -fy"]; 
        executeCommand(cmd2);
    }
